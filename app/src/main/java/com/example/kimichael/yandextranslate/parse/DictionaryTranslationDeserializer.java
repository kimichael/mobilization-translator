...
public class DictionaryTranslationDeserializer implements JsonDeserializer<DictionaryTranslation> {
    ...
    @Override
    public DictionaryTranslation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        if (!translationJson.has(DEFINITION_ARRAY))
            throw new JsonParseException("DictionaryTranslation has no definitions");
        ...
        dictionaryTranslation.setDefinitions(definitions);
        dictionaryTranslation.setTranslatedWord(definitions.get(0).getInterpretations().get(0).getDestWord());
